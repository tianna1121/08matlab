function h = myBiplot(coefs,varargin)
...
%%%% change by zhangyang, according to:http://www.yueye.org/2013/solve-biplot-obslabels-of-matlab.html
if 0
    if ~isempty(obslabs)
    ...
    if ~isempty(obslabs)
        [pp,dd] = size(scores);
        if ~(ischar(obslabs) && (size(obslabs,1) == pp)) && ...
                               ~(iscellstr(obslabs) && (numel(obslabs) == pp))
            error(message('stats:biplot:InvalidVarLabels'));
        end
        ...
        if in3D
            varTxtHndl = text(scores(:,1)+delx,scores(:,2)+dely,scores(:,3)+delz,obslabs);
        else
            varTxtHndl = text(scores(:,1)+delx,scores(:,2)+dely,obslabs);
        end
        set(varTxtHndl,'Tag','obslabel');
    end
%%%%
